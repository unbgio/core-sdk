...
fn main() -> Result<()> {
    ...
    let result = serde_json::json!({
        "tauriModel": format!("{:?}", tauri.model_used),
        "androidModel": format!("{:?}", android.model_used),
        "iosModel": format!("{:?}", ios.model_used),
        "tauriProvider": tauri.provider_selected,
        "androidProvider": android.provider_selected,
        "iosProvider": ios.provider_selected,
        "tauriMaskBytes": tauri.mask_png.len(),
        "androidMaskBytes": android.mask_png.len(),
        "iosMaskBytes": ios.mask_png.len(),
        "consistentMaskSize": tauri.mask_png.len() == android.mask_png.len() && android.mask_png.len() == ios.mask_png.len(),
    });
